...
@section('content')
    ...
    <form method="POST" action="{{ route('pembayaran.update', $pembayaran) }}">
        ...
        <div class="mb-3">
            ...
            <select name="kunjungan_id" id="kunjungan_id" class="form-control" required>
                ...
                @foreach ($kunjungan as $k)
                    <option value="{{ $k->id }}" {{ old('kunjungan_id', $pembayaran->kunjungan_id) == $k->id ? 'selected' : '' }}>{{ $k->pasien->nama }} ({{ \Carbon\Carbon::parse($k->tanggal_kunjungan)->format('d-m-Y H:i') }})</option>
                @endforeach
            </select>
            @error('kunjungan_id')
        ...
        <div class="mb-3">
            ...
            <input type="number" name="total_biaya" id="total_biaya" class="form-control" value="{{ old('total_biaya', $pembayaran->total_biaya) }}" required>
            @error('total_biaya')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="date" name="tanggal_bayar" id="tanggal_bayar" class="form-control" value="{{ old('tanggal_bayar', $pembayaran->tanggal_bayar) }}">
            @error('tanggal_bayar')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
