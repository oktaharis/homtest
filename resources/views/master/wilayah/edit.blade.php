...
<x-form-container 
    title="Edit Wilayah" 
    description="Perbarui informasi wilayah"
    :back-route="route('master.wilayah.index')"
    back-text="Kembali ke Daftar Wilayah">
    
    <form method="POST" action="{{ route('master.wilayah.update', $wilayah) }}" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <x-form-group label="Provinsi Induk" name="parent_id" :error="$errors->first('parent_id')">
                <x-form-select name="parent_id" placeholder="Pilih provinsi (opsional)">
                    @foreach ($provinsi as $prov)
                        <option value="{{ $prov->id }}" {{ old('parent_id', $wilayah->parent_id) == $prov->id ? 'selected' : '' }}>
                            {{ $prov->nama }}
                        </option>
                    @endforeach
                </x-form-select>
            ...
            <div class="flex">
                ...
                <div class="ml-3">
                    <p class="text-sm text-blue-700">
                        <strong>Catatan:</strong> Jika mengubah tipe menjadi "Kabupaten", pastikan untuk memilih provinsi induk. Untuk tipe "Provinsi", biarkan provinsi induk kosong.
                    </p>
                </div>
            </div>
        </div>
