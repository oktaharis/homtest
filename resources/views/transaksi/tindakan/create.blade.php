...
<x-form-container 
    ...
    description="Catat tindakan medis yang diberikan kepada pasien"
    :back-route="route('transaksi.tindakan.index')"
    back-text="Kembali ke Daftar Transaksi">
    
    <form method="POST" action="{{ route('transaksi.tindakan.store') }}" class="space-y-6">
        @csrf
            ...
            <x-form-group label="Jenis Tindakan" name="tindakan_id" required :error="$errors->first('tindakan_id')">
                <x-form-select name="tindakan_id" required placeholder="Pilih tindakan medis">
                    @foreach ($tindakan as $t)
                        <option value="{{ $t->id }}" {{ old('tindakan_id') == $t->id ? 'selected' : '' }}>
            ...
            <div class="flex">
                ...
                <div class="ml-3">
                    <p class="text-sm text-green-700">
                        <strong>Tips:</strong> Total biaya akan dihitung otomatis berdasarkan tarif tindakan dan jumlah yang dimasukkan. Catatan dapat membantu untuk dokumentasi medis yang lebih lengkap.
                    </p>
                </div>
            </div>
        </div>
